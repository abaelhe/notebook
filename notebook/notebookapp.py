...
def kernel_request(
    ...
    if log:
        log.debug("%s request to %s", method, fullurl)
...
class NotebookApp(JupyterApp):
    ...
    def init_configurables(self):
        ...
        if isinstance(self.kernel_manager, AsyncMappingKernelManager):
            if sys.version_info < (3, 6):  # Can be removed once 3.5 is dropped.
                raise ValueError(
                    "You are using `AsyncMappingKernelManager` in Python 3.5 (or lower) "
    ...
    def launch_browser(self):
        ...
        threading.Thread(target=br).start()
...
def list_running_servers(runtime_dir=None):
    ...
    for file_name in os.listdir(runtime_dir):
        if re.match("nbserver-(.+).json", file_name):
            ...
            # active check whether that process is really available via true HTTP request
            # Also remove leftover files from IPython 2.x without a pid field
            response = kernel_request(info, path="/login")
            if (
                response.code in ("200",)
                and response.body.find(b"<title>Jupyter Notebook</title>") > 0
            ):
                yield info
            else:
